...
const FAQ_ITEMS = [
  ...
  { q: "Can I see weekly and monthly attendance summaries?", a: "Yes. ATTENDEX rolls up daily clock-ins into weekly and monthly summaries per employee, team, or department, including hours worked, late arrivals, and absences — exportable as CSV or PDF." },
        ...
        {FAQ_ITEMS.map((item, i) => {
          ...
          return (
            <div className="border-b border-cdc-border py-4" key={item.q}>
              ...
              <div className={`text-xs text-gray-600 leading-[1.65] max-w-[600px] mt-3 ${isOpen ? "block" : "hidden"}`}>{item.a}</div>
            </div>
          );
        })}
      </div>
