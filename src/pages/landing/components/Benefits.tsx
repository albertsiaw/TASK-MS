...
const cards = [
  { icon: <IconClock />, title: "Daily Attendance Tracking", text: "Employees clock in and out with a tap. ATTENDX timestamps every entry and flags late arrivals automatically.", accent: false },
...
export default function Benefits() {
  return (
    <>
      <section className="py-10 pb-16 border-t border-cdc-border">
        <div className="max-w-[1180px] mx-auto px-[18px] md:px-6">
          <div className="grid grid-cols-1 lg:grid-cols-[5fr_7fr] gap-3.5 lg:gap-6 mb-[30px] items-center">
            <div>
              <span className="text-[10.5px] font-bold tracking-[0.12em] uppercase text-muted-lt block mb-2.5">WHY ATTENDX</span>
            ...
            <p className="text-[12px] sm:text-[13px] text-muted leading-[1.65] max-w-[500px]">From clock-in to task completion, ATTENDX keeps every record organized and ready for payroll or reporting.</p>
          </div>
            ...
            {cards.map((c) => (
              ...
              >
                <div>
                  ...
                  <p className={`text-[11.5px] leading-[1.55] ${c.accent ? "text-tan-ink" : "text-muted"}`}>{c.text}</p>
                </div>
